...
export const EN = {
  ...
  'td.takeOff': 'Take off {unit} · {slot}',
  'td.leavesBare': 'leaves that slot empty',
  'td.fromInventory': 'inventory',
  'td.wearingNothing': 'nothing',
  'td.itemLevel': 'lv {n}',
  'td.slotNumber': 'slot {n}',
  'td.clearLayout': 'Clear',
  'td.swapNote':
    'A move that takes a piece off a team-mate empties their slot, so the gain beside it is the whole squad’s, not the receiver’s — and what the receiver takes off goes back on the pile for a later line to use. A slot nothing was left to fill is marked.',
  ...
  'td.nodeBlurb':
    'Campaign nodes name no required units, so within whoever the campaign lets you field this is a squad picked for surviving the board. The “on this node” columns in the picker are each unit’s damage after these enemies’ armour, which is why a Psychic attacker outranks a bigger Physical one here — split into the weapon it swings every turn and the ability it usually gets one shot of.',
  ...
  'td.nodeOnly': 'node {node} ({slots} slots)',
  'td.campaignNode': '{campaign} node {node}',
